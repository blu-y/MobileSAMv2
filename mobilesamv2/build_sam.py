...
def build_sam_vit_t_encoder(checkpoint=None):
    ...
    if checkpoint is not None:
        with open(checkpoint, "rb") as f:
            state_dict = torch.load(f)
        new_state_dict = {k.replace('image_encoder.', ''): v for k, v in state_dict.items() if k.startswith('image_encoder.')}
        mobile_sam.load_state_dict(new_state_dict, strict=False)
    return mobile_sam
